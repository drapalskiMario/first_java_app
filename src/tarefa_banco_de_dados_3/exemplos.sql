...
FROM VENDA V
INNER JOIN CLIENTE C ON V.ID_CLIENTE_FK = C.ID
INNER JOIN PRODUTO_QUANTIDADE P ON P.ID_VENDA_FK = V.ID
WHERE V.CODIGO = 'A1';


SELECT PQ.ID, PQ.QUANTIDADE, PQ.VALOR_TOTAL,
P.ID AS ID_PRODUTO, P.CODIGO, P.NOME, P.DESCRICAO, P.VALOR
FROM PRODUTO_QUANTIDADE PQ
INNER JOIN PRODUTO P ON P.ID = PQ.ID_PRODUTO_FK;
...
ALTER TABLE PRODUTO
ADD COLUMN COR VARCHAR(255) NOT NULL;